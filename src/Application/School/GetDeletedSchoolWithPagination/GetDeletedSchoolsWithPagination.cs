    ...
    PaginatedList<SchoolItemDto>>
{
    ...
    public Task<PaginatedList<SchoolItemDto>> Handle(GetDeletedSchoolsWithPagination request, CancellationToken cancellationToken)
    {
        var query = _context.Schools.AsQueryable().Where(e => e.IsDeleted).IgnoreQueryFilters();
